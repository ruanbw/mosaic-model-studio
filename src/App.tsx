...
function App() {
    ...
    mutationFn: async ({ models, prompt: currentPrompt, demoMode: useDemo, signal }) => {
      ...
      return Promise.all(models.map(async (model, index) => {
        ...
        try {
          ...
          const result: GenerationResult = { id: runningResults[index].id, providerId: model.providerId, providerName: model.providerName, model: model.model, accent: model.accent, status: 'success', project: output.project, html: output.html, raw: output.raw, elapsedMs: Date.now() - startedAt, createdAt: Date.now() }
  ...
  const handleRetry = (result: GenerationResult) => { const model = modelOptions.find((item) => item.providerId === result.providerId && item.model === result.model); if (!model) { toast.error(t('models.removed')); return }; if (startGeneration([model], prompt, demoMode, true)) handleRemoveResult(result.id) }
  ...
  const handleRestore = () => { if (!window.confirm(t('settings.restoreConfirm'))) return; stopActiveProject(); restoreDefaults(); clearResults(); setPreviewResult(null); toast.success(t('settings.restored')) }
  const handleClear = () => { if (!window.confirm(t('settings.clearConfirm'))) return; stopActiveProject(); clearConfiguration(); setPreviewResult(null); toast.success(t('settings.cleared')) }
  const handleViewChange = (view: AppView) => { setActiveView(view); setMobileSidebarOpen(false) }
  const loadDemo = () => { const demoModels = selectedModels.length > 0 ? selectedModels : modelOptions; if (demoModels.length === 0) { toast.error(t('models.none')); return }; stopActiveProject(); replaceResults(createInitialDemoResults(demoModels)); setPreviewResult(null); toast.success(t('results.demoLoaded')) }

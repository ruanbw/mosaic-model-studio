  ...
  return (
    <section aria-labelledby="prompt-heading" aria-busy={isRunning}>
      <div className="mb-3 flex items-center justify-between gap-5">
        <div>
          <p className="eyebrow mb-1.5 font-mono text-[10px] uppercase tracking-[0.12em] text-faint">01 / DIRECTIVE</p>
          <h2 id="prompt-heading" className="text-lg font-medium tracking-[-0.03em]">{t('prompt.label')}</h2>
        </div>
      ...
      <div className="overflow-hidden rounded-[11px] border border-[#343a43] bg-surface shadow-[0_10px_35px_color-mix(in_srgb,#000_12%,transparent)] transition-[border-color,box-shadow] focus-within:border-mint/50 focus-within:shadow-[0_0_0_3px_color-mix(in_srgb,var(--mint)_6%,transparent)]">
        ...
        <div className="flex min-h-[49px] flex-col items-start justify-between gap-3 border-t border-line-soft px-3.5 pb-2.5 pt-2.5 min-[580px]:flex-row min-[580px]:items-center min-[580px]:px-5 min-[580px]:pb-2.5 min-[580px]:pt-2.5">
          <div className="flex flex-wrap items-center gap-1.5" aria-label={t('prompt.templatesLabel')}>
            <span className="mr-1 max-[580px]:w-full font-mono text-[10px] text-faint">{t('prompt.quickStart')}</span>
            {PROMPT_TEMPLATES.map((template) => <button className="min-h-9 rounded-md border border-[#30353d] bg-surface-soft px-2.5 py-1.5 text-[10px] text-muted transition-colors hover:border-mint/35 hover:text-mint" key={template.id} type="button" onClick={() => onPromptChange(t(template.promptKey))}>{t(`templates.${template.id}`)}</button>)}
          </div>
      ...
      <div className="mt-3 flex flex-col items-stretch justify-between gap-3 min-[580px]:flex-row min-[580px]:items-center">
        <button className="flex min-h-10 w-fit items-center gap-2 rounded-lg border border-transparent bg-transparent px-2 py-1.5 text-left transition-colors hover:border-line hover:bg-surface" type="button" onClick={onManageModels}>
          ...
          <span className="flex min-w-0 flex-col gap-0.5"><strong className="text-[11px] font-semibold text-ink">{selectedCount > 0 ? t('prompt.modelsReady', { count: selectedCount }) : t('prompt.selectModels')}</strong><small className="text-[10px] text-faint">{selectedCount > 1 ? t('prompt.parallel') : t('prompt.multiSelect')}</small></span>
          <ArrowUpRight className="ml-1 shrink-0 text-faint" size={15} />
        </button>

        <div className="flex min-h-[44px] flex-wrap items-center justify-between gap-2 min-[580px]:justify-end">
          ...
          <div className="flex w-full items-center gap-2 min-[580px]:w-auto min-[580px]:justify-end">
            {isRunning && onStop && <button className="inline-flex min-h-11 min-w-[82px] items-center justify-center gap-1.5 rounded-[7px] border border-red/35 bg-red/10 px-3 text-[11px] font-semibold text-red transition-colors hover:bg-red/20" type="button" onClick={onStop} aria-label={t('results.web.stop')}>
              <CircleStop size={16} />
              <span>{t('results.web.stop')}</span>
            </button>}
            <button className="inline-flex min-h-11 min-w-[137px] flex-1 items-center justify-center gap-2 rounded-[7px] border border-mint bg-mint px-3.5 text-[11px] font-semibold text-[#122018] shadow-[0_5px_18px_color-mix(in_srgb,var(--mint)_12%,transparent)] transition-colors hover:border-[#c0f7d9] hover:bg-[#c0f7d9] disabled:cursor-not-allowed disabled:opacity-50 min-[580px]:flex-none" type="button" onClick={onGenerate} disabled={isRunning || !prompt.trim() || selectedCount === 0}>

...
export function ProviderDialog({ open, onOpenChange, provider = null, onSave, onDelete }: ProviderDialogProps) {
  ...
  const { register, handleSubmit, reset, setValue, setError, getValues, watch, formState: { errors, isSubmitting } } = useForm<ProviderFormValues>({ resolver: zodResolver(providerDraftSchema), defaultValues: emptyValues })
  ...
  const requestModels = async (values: ProviderFormValues) => fetchProviderModels({ id: 'draft', name: values.name || 'Draft', kind: values.kind, apiKey: values.apiKey, baseUrl: values.baseUrl || undefined, models: [], accent: '#8ef0c4', enabled: true })
  const handleFetchModels = async () => {
    const values = getValues()
    if (!values.apiKey.trim()) { toast.error(t('dialog.apiKeyRequired')); return }
    setFetchingModels(true)
    try {
      const models = await requestModels(values)
  ...
  const submit = async (values: ProviderFormValues) => {
    ...
    if (!models && values.apiKey.trim()) {
      try {
        const fetchedModels = await requestModels(values)
        models = fetchedModels.join('\n')
        setValue('models', models, { shouldDirty: true })
      } catch (error) { toast.error(providerErrorMessage(error)); return }
    }
    if (!models) { setError('models', { message: t('validation.models') }); return }
    onSave({ ...values, models }, provider?.id); toast.success(provider ? t('dialog.updated') : t('dialog.added')); onOpenChange(false)
  }

...
interface ProvidersViewProps { providers: Provider[]; onAdd: (opener?: HTMLElement) => void; onEdit: (provider: Provider, opener?: HTMLElement) => void; onDelete: (provider: Provider) => void; providerDialogOpen?: boolean }
const providerInitials: Record<string, string> = { openai: '◎', anthropic: 'A', gemini: '✦', openrouter: 'OR' }

export function ProvidersView({ providers, onAdd, onEdit, onDelete, providerDialogOpen }: ProvidersViewProps) {
  ...
  return (
    <div className="mx-auto w-full max-w-[1120px] min-w-0 px-4 pb-20 pt-[52px] max-[580px]:px-[15px] max-[580px]:pt-8">
      <div className="mb-9 flex min-w-0 items-end justify-between gap-5 max-[580px]:flex-col max-[580px]:items-stretch">
        ...
        <button className="inline-flex min-h-11 shrink-0 items-center justify-center gap-2 rounded-md border border-mint bg-mint px-3.5 text-[11px] font-semibold text-[#122018] transition-colors hover:bg-[#c0f7d9] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-mint/70 focus-visible:ring-offset-2 focus-visible:ring-offset-canvas max-[580px]:w-full" type="button" onClick={(event) => onAdd(event.currentTarget)} aria-haspopup="dialog" aria-expanded={providerDialogOpen}><Plus aria-hidden="true" size={17} />{t('providers.add')}</button>
      </div>
      ...
      {providers.length === 0 ? (
        <div className="min-w-0 rounded-[10px] border border-dashed border-line bg-surface-soft px-5 py-[75px] text-center">
          ...
          <button className="inline-flex min-h-11 items-center justify-center gap-2 rounded-md border border-mint bg-mint px-3.5 text-[11px] font-semibold text-[#122018] transition-colors hover:bg-[#c0f7d9] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-mint/70" type="button" onClick={(event) => onAdd(event.currentTarget)} aria-haspopup="dialog" aria-expanded={providerDialogOpen}><Plus aria-hidden="true" size={16} />{t('providers.first')}</button>
        </div>
      ) : (

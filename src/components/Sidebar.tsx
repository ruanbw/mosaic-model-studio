...
function SidebarContent({ activeView, providerCount, configuredCount, onViewChange, onAddProvider, providerDialogOpen, onClose }: SidebarContentProps) {
  ...
  return <>
    ...
    <div className="mb-2 mt-7 flex min-h-11 items-center justify-between gap-2 px-2.5 font-mono text-[10px] uppercase tracking-[0.1em] text-faint"><span>{t('nav.providers')}</span><button className="grid size-11 shrink-0 place-items-center rounded-md border border-line text-muted transition-colors hover:border-mint/40 hover:text-mint focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-mint/70" type="button" onClick={(event) => { onAddProvider(event.currentTarget); onClose?.() }} aria-label={t('providers.add')} aria-haspopup="dialog" aria-expanded={providerDialogOpen}><Plus size={15} /></button></div>
...
export function Sidebar(props: SidebarProps) {
  const { activeView, providerCount, configuredCount, onViewChange, onAddProvider, providerDialogOpen, mobileOpen, onMobileOpenChange, returnFocusRef } = props
  const { t } = useTranslation()
  const contentProps = { activeView, providerCount, configuredCount, onViewChange, onAddProvider, providerDialogOpen }
  return <>
